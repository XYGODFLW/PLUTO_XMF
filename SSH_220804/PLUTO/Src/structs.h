...
typedef struct DATA{
  ...
  double nptc;// #mmd# how many kinds of particals included in this fluid
  double ****Vmx;// #mmd# component of the fluid, expressed by mass fraction
...
//MMD model
  char SED_file[256]; /* SED_file which is set as the xuv/euv input */
  char IC_file[256];  /* Initial boundary condition file, in short: IC */
  char obj_name[256]; /* Name of modeled object */
} Runtime;
...
typedef void UserDefBoundary (const Data *, RBox *, int,  Grid *);
typedef void Init (double *, double, double, double);
