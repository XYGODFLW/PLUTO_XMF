...
//if(!(g_stepNumber%1000))printf("%10.4e, %10.4e, %10.4e\n",d->Vc[RHO][0][0][50],d->Vc[VX1][0][0][50],d->Vc[PRS][0][0][50]);
//if(!(g_stepNumber%1000))printf("%10.4e, %10.4e, %10.4e\n",d->Uc[0][0][50][RHO],d->Uc[0][0][50][VX1],d->Uc[0][0][50][PRS]);
...
/* ********************************************************************* */
void hd_UpdateStage(const Data *d, Data_Arr UU, double **aflux,
 ...
 *********************************************************************** */
{
  ...
  int  *ip;
  double *inv_dl, dl2;
  static double ***T, ***C_dt[HD_NVAR], **dcoeff;
...
/* --------------------------------------------------------------
  ...
  if (state.v == NULL){
    MakeState (&state);
     ...
     dcoeff = ARRAY_2D(NMAX_POINT, HD_NVAR, double);
    #endif
  }
...
/* ----------------------------------------------------------------
  ...
  for (dir = beg_dir; dir <= end_dir; dir++){
    ...
    g_dir = dir;  
    hd_SetIndexes (&indx, grid);  /* -- set normal and transverse indices -- */
    ...
    TRANSVERSE_LOOP(indx,ip,i,j,k){
      ...
      for ((*ip) = 0; (*ip) < indx.ntot; (*ip)++) {
        HD_VAR_LOOP(nv) state.v[(*ip)][nv] = d->Vc[nv][k][j][i];
        state.flag[*ip] = d->flag[k][j][i];
      }
      CheckNaN (state.v, 0, indx.ntot-1,0);
      hd_States  (&state, indx.beg - 1, indx.end + 1, grid); 
      ...
      #endif
      hd_RightHandSide (&state, Dts, indx.beg, indx.end, dt, grid);

    /* -- update:  U = U + dt*R -- */

      #ifdef CHOMBO
       for ((*ip) = indx.beg; (*ip) <= indx.end; (*ip)++) { 
         HD_VAR_LOOP(nv) UU[nv][k][j][i] += state.rhs[*ip][nv];
       }
       SaveAMRFluxes (&state, aflux, indx.beg-1, indx.end, grid);
      #else
       for ((*ip) = indx.beg; (*ip) <= indx.end; (*ip)++) { 
         HD_VAR_LOOP(nv) UU[k][j][i][nv] += state.rhs[*ip][nv];
       }
      #endif
...
/* -------------------------------------------------------------------
  ...
  if(!(g_stepNumber%1000))printf("after%10.4e, %10.4e, %10.4e\n",d->Uc[0][0][50][RHO],d->Uc[0][0][50][VX1],d->Uc[0][0][50][PRS]);
/* -------------------------------------------------------------------
   6.update primary variable
  -------------------------------------------------------------------- */
 ...
 *********************************************************************** */
{
  ...
  if ((g_dir == IDIR) && (g_stretch_fact != 1.)) {
    for (i = beg; i <= end; i++) {
      HD_NVAR_LOOP(nv) state->flux[i][nv] *= g_stretch_fact;
    }
  }
  #if (CH_SPACEDIM == 3)
  if ((g_dir == JDIR) && (g_x3stretch != 1.)) {
    for (i = beg; i <= end; i++) {
     HD_NVAR_LOOP(nv) state->flux[i][nv] *= g_x3stretch;
    }
  }
  if ((g_dir == KDIR) && (g_x2stretch != 1.)) {
    for (i = beg; i <= end; i++) {
     HD_NVAR_LOOP(nv) state->flux[i][nv] *= g_x2stretch;
    }
  }  
  #endif
#endif
  ...
  if (g_dir == IDIR){
    ...
    for (i = beg; i <= end; i++) {
      HD_NVAR_LOOP(nv) state->flux[i][nv] *= area;
    }
  }
#endif
