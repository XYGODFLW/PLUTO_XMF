...
static void OutflowBound   (double ***, int, int, Grid *);
static void PeriodicBound  (double ***, int, int);
static void ReflectiveBound(double ***, int, int, int);
                           
/* ********************************************************************* */
void hd_Boundary (const Data *d, int idim, Grid *grid)
/*!
 ...
 *********************************************************************** */
{
  ...
  int  type[6], sbeg, send, vsign[HD_NVAR];
  int  par_dim[3] = {0, 0, 0};
  double ***q;
...
/* -------------------------------------------------
   ...
   UserDefBoundary_f1 (d, NULL, 0, grid);
  #endif
...
/* --------------------------------------------------------
   ...
   -------------------------------------------------------- */
// remove this setting in to data struct(beg)

// remove this setting in to data struct(end)
  ...
  for (is = sbeg; is <= send; is++){
    ...
    /* --------------------------------------------------------------
      ...
      UserDefBoundary_f1 (d, box, side[is], grid);//need to be redefined
    }
  }
 ...
 *********************************************************************** */
{
  ...
  int Bn, Bp, Bt;
   ...
   for (nv = 0; nv < HD_NVAR; nv++) vsign[nv] = 1.0;
   return;
  #endif
