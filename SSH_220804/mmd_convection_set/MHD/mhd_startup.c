...
/* ********************************************************************* */
void mhd_Startup (Data *d, Grid *G)
/*! 
 *
 *
 *
 ...
 *********************************************************************** */
{
  ...
  if (uprim == NULL){
    uprim = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
    ucons = ARRAY_2D(NMAX_POINT, MHD_NVAR, double); 
  }
...
/* --------------------------------------------------------------
  ...
  ITOT_LOOP(i) { 
    ...
    for (nv = MHD_NVAR; nv--; ) d->Vc[nv][k][j][i] = u_av[nv] = 0.0;
...
/*  ----------------------------------------------------------------
    ...
    for (nv = MHD_NVAR; nv--;  ) d->Vc[nv][k][j][i] = u_av[nv];
...
/* --------------------------------------------------------------------
  ...
  IDOM_LOOP(i){ x1 = GX->x[i];
    ...
    for (nv = MHD_NVAR; nv--;  ) us[nv] = d->Vc[nv][k][j][i];
