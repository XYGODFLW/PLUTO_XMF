...
//model Initialize
  ...
  mmd_model_output(&model, grd, &ini, 1.0, ini.output_dir);
...
/*
       ...
       print1 (", Nsts = %d",Dts[0].Nsts);
      #endif
      #if (PARABOLIC_FLUX & RK_CHEBYSHEV)
       print1 (", Nrkc = %d",Dts[0].Nrkc);
      #endif
*/
      print1 ("]\n");      
    }
...
/*
    FMHD_LOOP(f) scrh = dbl_min(scrh, mhd_NextTimeStep(Dts+f, ini+f, grd));
    FHD_LOOP(f)  scrh = dbl_min(scrh,  hd_NextTimeStep(Dts+f, ini+f, grd));
*/

 //   g_dt = MIN(g_dt, ini.cfl_max_var*scrh);//limit dt

    if (g_dt < ini.first_dt*1.e-9){
      print1 ("! NextTimeStep(): dt is too small (%12.6e). Cannot continue.\n", g_dt);
      QUIT_PLUTO(1);
    } 

    if (g_stepNumber <= 1 && (ini.first_dt > g_dt/ini.cfl)){
