...
/* *********************************************************************** */
void hd_RightHandSide (const State_1D *state, Time_Step *Dts, 
 ...
 ************************************************************************* */
{
  ...
  if (fvA == NULL) fvA = ARRAY_2D(NMAX_POINT, HD_NVAR, double);
#endif

  #if GEOMETRY != CARTESIAN
   if (fA == NULL) fA = ARRAY_2D(NMAX_POINT, HD_NVAR, double);
  #endif
...
/* -----------------------------------------------------
  ...
  if (g_dir == IDIR){
    ...
    /* -- I1. initialize rhs with flux difference -- */
      ...
      HD_NVAR_LOOP(nv) rhs[i][nv] = -dtdx*(flux[i][nv] - flux[i-1][nv]);
    ...
    /* -- J1. initialize rhs with flux difference -- */
      ...
      HD_NVAR_LOOP(nv) rhs[j][nv] = -dtdx*(flux[j][nv] - flux[j-1][nv]);
      #if USE_PR_GRADIENT == YES
    ...
    /* -- K1. initialize rhs with flux difference -- */
      ...
      HD_NVAR_LOOP(nv) rhs[k][nv] = -dtdx*(flux[k][nv] - flux[k-1][nv]);
      #if USE_PR_GRADIENT == YES
...

{
  ...
  if (g_dir == IDIR) {  
    ...
    hd_GetAreaFlux (state, fA, fvA, beg, end, grid);
    for (i = beg; i <= end; i++){ 
      R    = x1[i];
      dtdV = dt/dV1[i];
      dtdx = dt/dx1[i];
    ...
    /* -- I5. Add dissipative terms to entropy equation -- */
      ...
      #if (ENTROPY_SWITCH) && (PARABOLIC_FLUX & EXPLICIT)
       HD_NVAR_LOOP(nv) vc[nv] = 0.5*(vp[i][nv] + vm[i][nv]); 
       rhog = vc[RHO];
       rhog = (g_gamma - 1.0)*pow(rhog,1.0-g_gamma);
...
#elif GEOMETRY == POLAR
{
  ...
  if (g_dir == IDIR) { 
    ...
    /* -- I5. Add dissipative terms to entropy equation -- */
      ...
      #if (ENTROPY_SWITCH) && (PARABOLIC_FLUX & EXPLICIT)
       for (nv = HD_NVAR; nv--;  ) vc[nv] = 0.5*(vp[i][nv] + vm[i][nv]); 
       rhog = vc[RHO];
       rhog = (g_gamma - 1.0)*pow(rhog,1.0-g_gamma);
    ...
    /* -- K1. initialize rhs with flux difference -- */
      ...
      HD_VAR_LOOP(nv) rhs[k][nv] = -dtdx*(flux[k][nv] - flux[k-1][nv]);
      rhs[k][MX3] -= dtdx*(p[k] - p[k-1]);
...
dVdx = dV1[i]/dx1[i];
HD_NVAR_LOOP(nv) rhs[i][nv] = -dtdV*(fA[i][nv] - fA[i-1][nv]);
rhs[i][MX1] -= dtdx*(p[i] - p[i-1]);
rhs[i][MX3] *= r_1;
...
/* --------------------------------------------------------------
  ...
  hd_RightHandSideSource (state, Dts, beg, end, dt, phi_p, grid);
