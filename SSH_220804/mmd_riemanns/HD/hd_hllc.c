...
/* ********************************************************************* */
void hd_HLLC_Solver (const State_1D *state, int beg, int end, 
                  real *cmax, Grid *grid)
/*!
 ...
 *********************************************************************** */
{
  int    nv, i;
  double scrh, vxr, vxl;
  double usL[HD_NFLX], usR[HD_NFLX], vs;
  double qL, qR, wL, wR;
  double *vL, *vR, *uL, *uR;
  #if EOS == ISOTHERMAL
   double rho, mx;
...
/* ----------------------------------------------------
  ...
  hd_HLL_Speed (state->vL, state->vR, a2L, a2R, SL, SR, beg, end);
  ...
  for (i = beg; i <= end; i++) {
    ...
    if (SL[i] > 0.0){
      ...
      for (nv = HD_NFLX; nv--; ) state->flux[i][nv] = fL[i][nv];
      state->press[i] = pL[i];

    }else if (SR[i] < 0.0){

      for (nv = HD_NFLX; nv--; ) state->flux[i][nv] = fR[i][nv];
      state->press[i] = pR[i];
