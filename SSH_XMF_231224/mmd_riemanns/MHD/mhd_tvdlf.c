...
/* ********************************************************************* */
void mhd_LF_Solver (const State_1D *state, int beg, int end, 
 ...
 *********************************************************************** */
{
  ...
  if (fR == NULL){
    fR  = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
    fL  = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
    vRL = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
...
/* ------------------------------------------------------
  ...
  for (i = beg; i <= end; i++) {
    MHD_NVAR_LOOP(nv) vRL[i][nv] = 0.5*(VL[i][nv] + VR[i][nv]);
  ...
  mhd_MaxSignalSpeed (vRL, a2R, cmin_RL, cmax_RL, bgf, beg, end);
