...
#if HAVE_ENERGY
/* ********************************************************************* */
void mhd_HLLC_Solver (const State_1D *state, int beg, int end, 
 ...
 *********************************************************************** */
{
  ...
  double  scrh;
  double  pl, pr;
  double  vBl, usl[MHD_NFLX];
  double  vBr, usr[MHD_NFLX];

  double  Bxs, Bys, Bzs, ps, vBs;
  double  vxl, vxr, vxs, vys, vzs;
  double  Fhll[MHD_NFLX], alpha_l, alpha_r;
  ...
  if (fL == NULL){
    ...
    Uhll = ARRAY_2D(NMAX_POINT, MHD_NFLX, double);
...
/* -----------------------------------------------------
   ...
   mhd_HLL_DivBSource (state, Uhll, beg + 1, end, grid);
  #endif
}
