...
/*!
  ...
  The function mhd_MaxSignalSpeed() computes the maximum and minimum 
...
/* ********************************************************************* */
void mhd_MaxSignalSpeed (double **v, double *cs2, double *cmin, double *cmax,
...
/* ********************************************************************* */
void mhd_Eigenvalues(double **v, double *csound2, double **lambda, 
                 int beg, int end)
/*!
...
/* ********************************************************************* */
void mhd_PrimEigenvectors(double *q, double a2, double h, double *lambda,
                      double **LL, double **RR)
/*!
...
/* -------------------------------------------------------------------
  ...
  #if DIMENSIONS <= 2
  if (q[BXn] == 0 && q[BXt] == 0.0) for (k = 0; k < MHD_NFLX; k++) RR[BXt][k] = 0.0;
  #endif
#endif
...
#if CHECK_EIGENVECTORS == YES
{
  ...
  if (A == NULL){
    A   = ARRAY_2D(MHD_NFLX, MHD_NFLX, double);
    ALR = ARRAY_2D(MHD_NFLX, MHD_NFLX, double);
    #if COMPONENTS != 3
 ...
 /* --------------------------------------
  ...
  for (i = 0; i < MHD_NFLX; i++){
  for (j = 0; j < MHD_NFLX; j++){
    A[i][j] = ALR[i][j] = 0.0;
  }}
  ...
  /* --------------------------------------------------------
    ...
    if (fabs(dA) > 1.e-8){
      ...
      print ("\n\n A = \n");   ShowMatrix(A, MHD_NFLX, 1.e-8);
      print ("\n\n R.Lambda.L = \n"); ShowMatrix(ALR, MHD_NFLX, 1.e-8);
      QUIT_PLUTO(1);
    }
  }}  
...
/* -- check orthornomality -- */
  ...
  for (j = 0; j < MHD_NFLX; j++){
    ...
    #endif
    a = 0.0;
    for (k = 0; k < MHD_NFLX; k++) a += LL[i][k]*RR[k][j];
...
/* ********************************************************************* */
void mhd_ConsEigenvectors (double *u, double *v, double a2, 
...
/* -- check orthornomality -- */
  ...
  for (j = 0; j < MHD_NFLX; j++){
    ...
    #endif
    a = 0.0;
    for (k = 0; k < MHD_NFLX; k++) a += Lc[i][k]*Rc[k][j];
    if ( (i == j && fabs(a-1.0) > 1.e-8) || 
         (i != j && fabs(a)>1.e-8) ) {
...
/* ********************************************************************* */
void mhd_PrimToChar (double **Lp, double *v, double *w)
/*!
...
#if CHECK_EIGENVECTORS == YES
{
  ...
  for (k = 0; k < MHD_NVAR; k++){
    w2[k] = 0.0;
    for (i = 0; i < MHD_NVAR; i++) w2[k] += Lp[k][i]*v[i];
