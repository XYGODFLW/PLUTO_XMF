...
/* --------------------------------------------
   ...
   bgf = mhd_GetBackgroundField (beg - 1, end, CELL_CENTER, grid);
  #endif
 ...
 *********************************************************************** */
{
  ...
  double dxmin, gmaxc;
   ...
   if (glm_ch < 0.0){
     ...
     if (v == NULL) {
       ...
       lambda = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
  ...
  /* -- X1 - g_dir -- */
     ...
     g_dir = IDIR;  mhd_SetIndexes (&indx, grid);
     KDOM_LOOP(k) JDOM_LOOP(j){
       g_j = j; g_k = k;
       IDOM_LOOP(i) for (nv = MHD_NVAR; nv--;  ) v[i][nv] = d->Vc[nv][k][j][i];
       SoundSpeed2  (v, cs2, NULL, IBEG, IEND, CELL_CENTER, grid);
       mhd_Eigenvalues  (v, cs2, lambda, IBEG, IEND);
       IDOM_LOOP(i){
  ...
  /* -- X2 - g_dir -- */
     ...
     #if DIMENSIONS >= 2
     g_dir = JDIR;  mhd_SetIndexes (&indx, grid);
     KDOM_LOOP(k) IDOM_LOOP(i){
       g_i = i; g_k = k;
       JDOM_LOOP(j) for (nv = MHD_NVAR; nv--;  ) v[j][nv] = d->Vc[nv][k][j][i];
       SoundSpeed2  (v, cs2, NULL, JBEG, JEND, CELL_CENTER, grid);
       mhd_Eigenvalues  (v, cs2, lambda, JBEG, JEND);
       JDOM_LOOP(j){
  ...
  /* -- X3 - g_dir -- */
     ...
     #if DIMENSIONS == 3
     g_dir = KDIR;  mhd_SetIndexes (&indx, grid);
     JDOM_LOOP(j) IDOM_LOOP(i){
       g_i = i; g_j = j;
       KDOM_LOOP(k) for (nv = MHD_NVAR; nv--;  ) v[k][nv] = d->Vc[nv][k][j][i];
       SoundSpeed2  (v, cs2, NULL, KBEG, KEND, CELL_CENTER, grid);
       mhd_Eigenvalues  (v, cs2, lambda, KBEG, KEND);
       KDOM_LOOP(k){
