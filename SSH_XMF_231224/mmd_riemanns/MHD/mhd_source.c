...
/* ********************************************************************* */
void mhd_Roe_DivBSource (const State_1D *state, int is, int ie, Grid *grid)
/*!
...
/* --------------------------------------------
   ...
   bgf = mhd_GetBackgroundField (is - 1, ie, CELL_CENTER, grid);
  #endif
...
/* *********************************************************************  */
void mhd_HLL_DivBSource (const State_1D *state, double **Uhll, 
...
/* --------------------------------------------
  ...
  for (i = beg; i <= end; i++) {
    ...
    src = state->src[i];
    for (nv = MHD_NFLX; nv--;  ) vc[nv] = state->vh[i][nv];
