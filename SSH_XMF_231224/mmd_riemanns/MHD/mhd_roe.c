...
/* ********************************************************************* */
void mhd_Roe_Solver (const State_1D *state, int beg, int end, 
 ...
 *********************************************************************** */
{
  ...
  double dV[MHD_NFLX], dU[MHD_NFLX], *vL, *vR, *uL, *uR;
  double *SL, *SR;
  double Rc[MHD_NFLX][MHD_NFLX], eta[MHD_NFLX], lambda[MHD_NFLX];
  double alambda[MHD_NFLX], Uv[MHD_NFLX];
...
/* -----------------------------------------------------------
  ...
  if (fL == NULL){
    ...
    fL = ARRAY_2D(NMAX_POINT, MHD_NFLX, double);
    fR = ARRAY_2D(NMAX_POINT, MHD_NFLX, double);
    ...
    #ifdef GLM_MHD
     VL = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
     VR = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
     UL = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
     UR = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
    #endif
  }
...
/* -----------------------------------------------------------
  ...
  #if BACKGROUND_FIELD == YES
   bgf = mhd_GetBackgroundField (beg, end, FACE_CENTER, grid);
...
/* -----------------------------------------------------------
   ...
   mhd_PrimToCons (VL, UL, beg, end);
   mhd_PrimToCons (VR, UR, beg, end);
  #else
...
/* ----------------------------------------------------
  ...
  mhd_Flux (UL, VL, a2L, bgf, fL, pL, beg, end);
  mhd_Flux (UR, VR, a2R, bgf, fR, pR, beg, end);
...
/* ---------------------------------------------------------------------
  ...
  /* -----------------------------------------------------------
     ...
     if ((state->flag[i] & FLAG_HLL) || (state->flag[i+1] & FLAG_HLL)){
       mhd_HLL_Speed (VL, VR, a2L, a2R, NULL, SL, SR, i, i);

       scrh = MAX(fabs(SL[i]), fabs(SR[i]));
       cmax[i] = scrh;

       if (SL[i] > 0.0) {
         for (nv = MHD_NFLX; nv--; ) state->flux[i][nv] = fL[i][nv];
         state->press[i] = pL[i];
       } else if (SR[i] < 0.0) {
         for (nv = MHD_NFLX; nv--; ) state->flux[i][nv] = fR[i][nv];
         state->press[i] = pR[i];
       }else{
         scrh = 1.0/(SR[i] - SL[i]);
         for (nv = MHD_NFLX; nv--; ){
...
/*      sl = sr = 0.5;    */
  ...
  /* -----------------------------------------------------------------
   ...
   /* -----------------------------------------
    ...
    for (k = 0; k < MHD_NFLX; k++) alambda[k] = fabs(lambda[k]);
...
/* --------------------------------------------------------
  ...
  #if DIVB_CONTROL == EIGHT_WAVES
   mhd_Roe_DivBSource (state, beg + 1, end, grid);
  #endif
