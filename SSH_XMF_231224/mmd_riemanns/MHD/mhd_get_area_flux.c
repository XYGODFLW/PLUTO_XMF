...
/* ********************************************************************* */
void mhd_GetAreaFlux (const State_1D *state, double **fA,
 ...
 *********************************************************************** */
{
  ...
  if (g_dir == IDIR) { 
    ...
    for (i = beg - 1; i <= end; i++){ 
      ...
      MHD_NVAR_LOOP(nv) fA[i][nv] = flux[i][nv]*A;
  #if COMPONENTS == 3    
      ...
      rp2 = rp*rp;
      MHD_NVAR_LOOP(nv) fA[i][nv] = flux[i][nv]*rp2;
  #if COMPONENTS == 3
      rp3 = rp2*rp;
    ...
    for (j = beg - 1; j <= end; j++){
      ...
      MHD_NVAR_LOOP(nv) fA[j][nv] = flux[j][nv]*s;
  #if COMPONENTS == 3
    ...
    print1 ("! mhd_GetAreaFlux(): should not be called during x3 sweep\n");
    QUIT_PLUTO(1);
