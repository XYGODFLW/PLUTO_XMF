...
/* *********************************************************************** */
void mhd_RightHandSideSource (const State_1D *state, Time_Step *Dts,
 ...
 ************************************************************************* */
{
  ...
  Bg0 = mhd_GetBackgroundField (beg, end, CELL_CENTER, grid);
#endif
...
/* --------------------------
  ...
  if (g_dir == IDIR){
    ...
    /* --------------------------------------------
      ...
      r_1 = 1.0/x1[i];
      vg  = vc;
      MHD_NFLX_LOOP(nv) vc[nv] = 0.5*(vp[i][nv] + vm[i][nv]);       
      #if COMPONENTS == 3
       vphi = vc[iVPHI];
    ...
    /* --------------------------------------------
      ...
      vg = vc;
      MHD_NFLX_LOOP(nv) vc[nv] = state->vh[j][nv];
      vphi = SELECT(0.0, 0.0, vc[iVPHI]);
  #if (defined FARGO) || (ROTATING_FRAME == YES)
      w = 0.0; 
      IF_FARGO         (w += wA[j][i];)
