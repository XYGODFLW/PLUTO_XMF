...
/* ********************************************************************* */
void hd_Roe_Solver (const State_1D *state, int beg, int end, 
 ...
 *********************************************************************** */
{
  int   nv, i, j, k, nn;
  double  scrh;
  double  um[HD_NFLX], vel2;
  double  a2, a, h;
  double  dv[HD_NFLX], eta[HD_NFLX];
  double  Rc[HD_NFLX][HD_NFLX], alambda[HD_NFLX], lambda[HD_NFLX];
  ...
  if (fL == NULL){
    fL  = ARRAY_2D(NMAX_POINT, HD_NFLX, double);
    fR  = ARRAY_2D(NMAX_POINT, HD_NFLX, double);
...
/* ----------------------------------------------------
  ...
  hd_Flux (state->uL, state->vL, a2L, fL, pL, beg, end);
  hd_Flux (state->uR, state->vR, a2R, fR, pR, beg, end);
  ...
  /*  ----  Define Wave Jumps  ----  */
    ...
    for (nv = HD_NFLX; nv--;   ) dv[nv] = qr[nv] - ql[nv];
  ...
  /* -------------------------------------------------
     ...
     #endif
    #else
     for (nv = HD_NFLX; nv--;   ) um[nv] = 0.5*(ql[nv] + qr[nv]);  
     #if EOS == IDEAL
  ...
  /* -----------------------------------------------------------
    ...
    for (nv = HD_NFLX; nv--;   ) alambda[nv]  = fabs(lambda[nv]);
