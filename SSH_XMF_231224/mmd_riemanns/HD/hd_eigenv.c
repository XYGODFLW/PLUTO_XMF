...
/* ********************************************************************* */
void hd_MaxSignalSpeed (double **v, double *cs2, double *cmin, double *cmax,
                     int beg, int end)
/*!
...
/* ********************************************************************* */
void hd_Eigenvalues(double **v, double *csound2, double **lambda, int beg, int end)
/*!
 ...
 *********************************************************************** */
{
  ...
  for (i = beg; i <= end; i++){
    ...
    for (k = 2; k < HD_NFLX; k++) lambda[i][k] = v[i][VXn];
  } 
}

/* ********************************************************************* */
void  hd_PrimEigenvectors (double *q, double cs2, double h, double *lambda,
                        double **LL, double **RR)
/*!
 ...
 *********************************************************************** */
{
  ...
  double Aw1[HF_NFLX], Aw0[HD_NFLX], AA[HD_NFLX][HD_NFLX], a;
#endif
...
/* ------------------------------------------------------
  ...
  for (k = 2; k < HD_NFLX; k++) lambda[k] = q[VXn];  /*  lambda = u   */
...
#if CHECK_EIGENVECTORS == YES
{
  ...
  if (A == NULL){
    A   = ARRAY_2D(HD_NFLX, HD_NFLX, double);
    ALR = ARRAY_2D(HD_NFLX, HD_NFLX, double);
    #if COMPONENTS != 3
 ...
 /* --------------------------------------
  ...
  for (j = 0; j < HD_NFLX; j++){
    ALR[i][j] = 0.0;
    for (k = 0; k < HD_NFLX; k++) ALR[i][j] += RR[i][k]*lambda[k]*LL[k][j];
  }}
  ...
  for (j = 0; j < HD_NFLX; j++){
    ...
    if (fabs(dA) > 1.e-8){
      ...
      print ("! cs2 = %12.6e\n",cs2);
      print ("\n\n A = \n");   ShowMatrix(A, HD_NFLX, 1.e-8);
      print ("\n\n R.Lambda.L = \n"); ShowMatrix(ALR, HD_NFLX, 1.e-8);
      QUIT_PLUTO(1);
    }
  }}  
...
/* -- check orthornomality -- */
  ...
  for (j = 0; j < HD_NFLX; j++){
    a = 0.0;
    for (k = 0; k < HD_NFLX; k++) a += LL[i][k]*RR[k][j];
...
/* ********************************************************************* */
void hd_ConsEigenvectors (double *u, double *v, double a2, 
...
/*       lambda = u       */
   ...
   for (k = 0; k < HD_NFLX; k++){   /* normalization */
   for (nv = 0; nv < HD_NFLX; nv++){
 ...
 /* --------------------------------------
  ...
  for (j = 0; j < HD_NFLX; j++){
    if (fabs(ALR[i][j] - A[i][j]) > 1.e-6){
      ...
      print ("\n\n RR   = \n"); ShowMatrix(RR, HD_NFLX, 1.e-8);
      print ("\n\n LL   = \n"); ShowMatrix(LL, HD_NFLX, 1.e-8);
      QUIT_PLUTO(1);
    }
  }}
...
/* -- check orthornomality -- */
  ...
  for (j = 0; j < HD_NFLX; j++){
    dA = 0.0;
    for (k = 0; k < HD_NFLX; k++) dA += LL[i][k]*RR[k][j];
