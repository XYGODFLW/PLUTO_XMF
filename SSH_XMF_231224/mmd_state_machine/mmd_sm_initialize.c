...
void mmd_sm_initialize(Runtime* ini)
{
  ...
  //initialize for sm_d
  sm_data.Vc = ARRAY_4D(MHD_NVAR, NX3_TOT, NX2_TOT, NX1_TOT, double);
  sm_data.Uc = ARRAY_4D(NX3_TOT, NX2_TOT, NX1_TOT, MHD_NVAR, double); 
  ...
  #if RESISTIVITY != NO
   sm_data.J = ARRAY_4D(DIMENSIONS,NX3_TOT, NX2_TOT, NX1_TOT, double);
  #endif

  sm_data.flag = ARRAY_3D(NX3_TOT, NX2_TOT, NX1_TOT, unsigned char);

  sm_du = ARRAY_4D(MHD_NVAR, NX3_TOT, NX2_TOT, NX1_TOT, double);
  return;
}
