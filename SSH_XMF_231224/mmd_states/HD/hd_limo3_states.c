...
/* ********************************************************************* */
void hd_States (const State_1D *state, int beg, int end, Grid *grid)
/* 
 *
 *
 *********************************************************************** */
{
  ...
  double **L, **R, *lambda;
  double dwp[HD_NVAR], dwp_lim[HD_NVAR];
  double dwm[HD_NVAR], dwm_lim[HD_NVAR];
  double dvpR, dvmR;
  static double **dv;
...
/* ----------------------------------------------------
  ...
  if (dv == NULL) {
    dv = ARRAY_2D(NMAX_POINT, HD_NVAR, double);
  }
...
/* ----------------------------------------------------
   ...
   i = beg-1;
   HD_NVAR_LOOP(nv) dv[i][nv] = v[i+1][nv] - v[i][nv];
  ...
  /* -------------------------------
     ...
     hd_PrimEigenvectors (v[i], state->a2[i], state->h[i], lambda, L, R);
     hd_PrimToChar(L, dvp, dwp);
     hd_PrimToChar(L, dvm, dwm);
...
/* ----------------------------------------------
  ...
  hd_PrimToCons (state->vp, state->up, beg, end);
  hd_PrimToCons (state->vm, state->um, beg, end);
}
