...
/* ********************************************************************* */
void mhd_States (const State_1D *state, int beg, int end, Grid *grid)
/* 
 *
 *
 *********************************************************************** */
{
  ...
  double **L, **R, *lambda;
  double dwp[MHD_NVAR], dwp_lim[MHD_NVAR];
  double dwm[MHD_NVAR], dwm_lim[MHD_NVAR];
  double dvpR, dvmR;
  static double **dv;
...
/* ----------------------------------------------------
  ...
  if (dv == NULL) {
    dv = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
  }
...
/* ----------------------------------------------------
   ...
   i = beg-1;
   MHD_NVAR_LOOP(nv) dv[i][nv] = v[i+1][nv] - v[i][nv];
  ...
  /* -------------------------------
     ...
     mhd_PrimEigenvectors (v[i], state->a2[i], state->h[i], lambda, L, R);
     mhd_PrimToChar(L, dvp, dwp);
     mhd_PrimToChar(L, dvm, dwm);
  ...
  /* -------------------------------------- 
    ...
    #if MHD_NFLX != MHD_NVAR
     for (nv = MHD_NFLX; nv < MHD_NVAR; nv++ ){
...
/* ----------------------------------------------
  ...
  mhd_PrimToCons (state->vp, state->up, beg, end);
  mhd_PrimToCons (state->vm, state->um, beg, end);
}
