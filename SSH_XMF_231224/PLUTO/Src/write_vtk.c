...
/* ********************************************************************* */
void WriteVTK_Vector (FILE *fvtk, double ****vect, double unit,
                      char *vect_name, Grid *grid)
/*!
 ...
 * \param [in]    grid  pointer to an array of Grid structures
 * adjusted for XMF (2023 09 by xing)
 *********************************************************************** */
{
  int i,j,k;
  char header[512];
  ...
  if (vect3D == NULL){
    vect3D = ARRAY_3D(KNP_GLOB, JNP_GLOB, INP_GLOB, Float_Vect);
  }
...
//   --------------------------------------------------------
  ...
  G_DOM_LOOP(k,j,i){ 
    D_EXPAND(v[0] = vect[0][k][j][i]; x1 = grid[IDIR].x[i]; ,
             v[1] = vect[1][k][j][i]; x2 = grid[JDIR].x[j]; ,
             v[2] = vect[2][k][j][i]; x3 = grid[KDIR].x[k];)
  ...
  sprintf (header,"\nVECTORS %dD_%s_vector float\n", DIMENSIONS, vect_name);
 ...
 * adjusted for XMF (20230831 by xing)
 *********************************************************************** */
{
  int i,j,k;
  char header[512];
