...
//MMD settings beg
/* ---- set SED_input file ---- */
  sprintf (runtime->SED_file, "%s", "SED_lists/y_4.4.dat");
  if (ParamExist("SED_FILE")){
    str = ParamFileGet("SED_FILE",1);
    sprintf (runtime->SED_file, "%s",str);
  }
/* ---- set IC_input file ---- */
  if (ParamExist("IC_FILE")){
    str = ParamFileGet("IC_FILE",1);
    sprintf (runtime->IC_file, "%s",str);
  }
...
/* ---- set OBJ_NAME ---- */
  if (ParamExist("OBJ_NAME")){
    str = ParamFileGet("OBJ_NAME",1);
    sprintf (runtime->obj_name, "%s",str);
  }
