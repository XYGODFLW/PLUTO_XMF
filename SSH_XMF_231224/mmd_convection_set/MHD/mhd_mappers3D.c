...
/* ********************************************************************* */
void mhd_ConsToPrim3D (Data_Arr U, Data_Arr V, unsigned char ***flag, RBox *box)
/*!
 ...
 *********************************************************************** */
{
  ...
  if (v == NULL){
    v = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
    u = ARRAY_2D(NMAX_POINT, MHD_NVAR, double);
  }
...
/* -----------------------------------------------
...
#ifdef CHOMBO
    for (i = ibeg; i <= iend; i++) MHD_NVAR_LOOP(nv) u[i][nv] = U[nv][k][j][i];
    #if COOLING == MINEq || COOLING == H2_COOL
    if (g_intStage == 1) for (i = ibeg; i <= iend; i++) NormalizeIons(u[i]);
    #endif
    err = mhd_ConsToPrim (u, v, ibeg, iend, flag[k][j]);
  ...
  /* -------------------------------------------------------------
    ...
    for (i = ibeg; i <= iend; i++) MHD_NVAR_LOOP(nv) U[nv][k][j][i] = u[i][nv];
#else
    err = mhd_ConsToPrim (U[k][j], v, ibeg, iend, flag[k][j]);
#endif
    for (i = ibeg; i <= iend; i++) MHD_NVAR_LOOP(nv) V[nv][k][j][i] = v[i][nv];
  }}
  g_dir = current_dir;

}
/* ********************************************************************* */
void mhd_PrimToCons3D (Data_Arr V, Data_Arr U, RBox *box)
/*!
...
/* -----------------------------------------------
  ...
  for (k = kbeg; k <= kend; k++){ g_k = k;
  for (j = jbeg; j <= jend; j++){ g_j = j;
    for (i = ibeg; i <= iend; i++) MHD_VAR_LOOP(nv) v[i][nv] = V[nv][k][j][i];
#ifdef CHOMBO
    mhd_PrimToCons(v, u, ibeg, iend);
    for (i = ibeg; i <= iend; i++) MHD_VAR_LOOP(nv) U[nv][k][j][i] = u[i][nv];
#else      
    mhd_PrimToCons (v, U[k][j], ibeg, iend);
#endif
  }}
