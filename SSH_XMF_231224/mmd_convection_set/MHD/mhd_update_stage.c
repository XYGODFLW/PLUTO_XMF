...
/* ********************************************************************* */
void mhd_UpdateStage(const Data *d, Data_Arr UU, double **aflux,
...
/* -----------------------------------------------
   2c. Vs->Vc using in multi-fluid model(Just Temporary)
   ----------------------------------------------- */
   ...
   CPN_LOOP(nv)TOT_LOOP(k,j,i) d->Vc[BX1+nv][k][j][i] = d->Uc[k][j][i][BX1+nv]; 
  #endif
...
/* ----------------------------------------------------------------
  ...
  for (dir = beg_dir; dir <= end_dir; dir++){
    ...
    g_dir = dir;  
    mhd_SetIndexes (&indx, grid);  /* -- set normal and transverse indices -- */
    ...
    TRANSVERSE_LOOP(indx,ip,i,j,k){
      ...
      for ((*ip) = 0; (*ip) < indx.ntot; (*ip)++) {
        MHD_NVAR_LOOP(nv) state.v[(*ip)][nv] = d->Vc[nv][k][j][i];
      ...
      mhd_CheckNaN (state.v, 0, indx.ntot-1,0);

      mhd_States  (&state, indx.beg - 1, indx.end + 1, grid); 
      ...
      mhd_RightHandSide (&state, Dts, indx.beg, indx.end, dt, grid);
    ...
    /* -- compute inverse dt coefficients when g_intStage = 1 -- */
      ...
      for ((*ip) = indx.beg; (*ip) <= indx.end; (*ip)++) { 
          ...
          C_dt[0][k][j][i] += 0.5*(Dts->cmax[(*ip)-1] + Dts->cmax[*ip])
                                 *inv_dl[*ip];
         #endif
         #if (PARABOLIC_FLUX & EXPLICIT)
...
/*
  ...
  if(mynode==0&&(g_stepNumber==0) ){
    printf("this node = %d\n", mynode);
    MHD_NVAR_LOOP(nv){
      printf("var%d\n", nv);
      DOM_LOOP(k,j,i){ 
        if(i==IBEG)printf("\n");
        printf("%.4e ", d->Uc[k][j][i][nv]);
      }
      printf("\n");
    }
  }
  ...
  //data check beg//check UL UR
  int totalnodes, mynode;
  MPI_Comm_size(MPI_COMM_WORLD, &totalnodes);
  MPI_Comm_rank(MPI_COMM_WORLD, &mynode);  

  if(mynode==0&&(g_stepNumber==0)) for ((*ip) = 0; (*ip) < indx.ntot; (*ip)++) 
  {
    if((*ip)==0)printf("\n");
    printf("%.4e ", state.flux[(*ip)][VX1]);
  }
  //data check end
*/
