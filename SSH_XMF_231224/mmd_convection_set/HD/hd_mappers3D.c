...
/* ********************************************************************* */
void hd_ConsToPrim3D (Data_Arr U, Data_Arr V, unsigned char ***flag, RBox *box)
/*!
 ...
 *********************************************************************** */
{
  ...
  if (v == NULL){
    v = ARRAY_2D(NMAX_POINT, HD_NVAR, double);
    u = ARRAY_2D(NMAX_POINT, HD_NVAR, double);
  }
...
/* -----------------------------------------------
...
#ifdef CHOMBO
    for (i = ibeg; i <= iend; i++) HD_NVAR_LOOP(nv) u[i][nv] = U[nv][k][j][i];
    #if COOLING == MINEq || COOLING == H2_COOL
    if (g_intStage == 1) for (i = ibeg; i <= iend; i++) NormalizeIons(u[i]);
    #endif
    err = hd_ConsToPrim (u, v, ibeg, iend, flag[k][j]);
  ...
  /* -------------------------------------------------------------
    ...
    for (i = ibeg; i <= iend; i++) HD_NVAR_LOOP(nv) U[nv][k][j][i] = u[i][nv];
#else
    err = hd_ConsToPrim (U[k][j], v, ibeg, iend, flag[k][j]);
#endif
    for (i = ibeg; i <= iend; i++) HD_NVAR_LOOP(nv) V[nv][k][j][i] = v[i][nv];
  }}
  g_dir = current_dir;

}
/* ********************************************************************* */
void hd_PrimToCons3D (Data_Arr V, Data_Arr U, RBox *box)
/*!
...
/* -----------------------------------------------
  ...
  for (k = kbeg; k <= kend; k++){ g_k = k;
  for (j = jbeg; j <= jend; j++){ g_j = j;
    for (i = ibeg; i <= iend; i++) HD_VAR_LOOP(nv) v[i][nv] = V[nv][k][j][i];
#ifdef CHOMBO
    hd_PrimToCons(v, u, ibeg, iend);
    for (i = ibeg; i <= iend; i++) HD_VAR_LOOP(nv) U[nv][k][j][i] = u[i][nv];
#else      
    hd_PrimToCons (v, U[k][j], ibeg, iend);
#endif
  }}
