...
void mmd_initialize_public(int argc, char *argv[], Runtime *runtime, Grid *grid, Cmd_Line *cmd_line)
{
/* *******************************
   parse pluto.ini and commandline
   Initialize computational grid, domain decomposition 
   prepare for paraller computing
   (part of original initialize.c) 
...
/* ------------------------------------------------
  ...
  KNP_GLOB = grid[KDIR].np_int_glob;
  JNP_GLOB = grid[JDIR].np_int_glob;
  INP_GLOB = grid[IDIR].np_int_glob;
